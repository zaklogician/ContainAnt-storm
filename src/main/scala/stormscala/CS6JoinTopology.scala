...
object CS6JoinTopology {
  ...
  object Hmma extends AntHeuristic {
    ...
    override val _iterations: Int = 10
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("\n-----------------------------")
    println("Case Study 6: Apache Storm - Join Topology")
    println("Runs: " + _runs)
    
    val comparison =
      Framework.experimentN[JoinConfig](Seq(Hmct, Hgre, Hmma),_runs, JoinModule, JoinTopology.runConfig)
    
    val reference =
      Framework.experiment[JoinConfig](Hran, Hran, _runs, JoinModule, JoinTopology.runConfig)
